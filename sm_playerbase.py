...
Updates all player dimensiontables including coaches, players for each squad, injury and transfer history of each player
"""
...
#aktueller_fantasy_spieltag = 19

response = requests.get("https://soccer.sportmonks.com/api/v2.0/teams/season/"+str(aktuelle_buli_season)+"?api_token="+my_token+"&include=coach,squad.player.sidelined,squad.player.transfers")
print("API CALL Squads: " + str(response.status_code))
...
# iterate through the squad of each team
for team in squad_response:
    ...
    for player in team['squad']['data']:
        ...
        # iterate through past sidelined-data (injuries, redcards etc)
        ...
        # iterate through transfer data of each player
        for transfer in player['player']['data']['transfers']['data']:
            ...
            #transfer_list.append(transfer['season_id'])
...
# data cleaning
df_squads[df_squads.duplicated(subset=['id'])]
